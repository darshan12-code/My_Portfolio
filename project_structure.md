...
Full-stack portfolio built with React + Flask. The frontend is a Vite/React SPA with styled-components and framer-motion. The backend is a Flask REST API with PostgreSQL via SQLAlchemy, JWT auth, and Cloudinary for media uploads.
...
If you're using Supabase, grab the pooled connection string from your project dashboard and use port `6543`.

### 3. Set up the database

The app uses a custom `app_schema` schema — not the default `public`. You need to create it first:

```sql
CREATE SCHEMA app_schema;
```

Then run the table migrations:
...
There's no registration UI. Drop into a Python shell and create one manually:
...
This inserts 5 case studies, 5 blog posts, and 3 contact messages so you have something to look at in the admin panel.
...
The API runs on `http://127.0.0.1:5000`. Check `http://127.0.0.1:5000/health` to confirm it's up.
...
For production, swap this for your deployed backend URL.
...
Output goes to `dist/`. The `vercel.json` already handles client-side routing rewrites, so Vercel deploys work out of the box.
...
| `SECRET_KEY` | backend | Flask + JWT signing secret |
| `CLOUDINARY_CLOUD_NAME` | backend | Cloudinary project name |
| `CLOUDINARY_API_KEY` | backend | Cloudinary API key |
| `CLOUDINARY_API_SECRET` | backend | Cloudinary API secret |
| `CONTACT_SENDER_EMAIL` | backend | From address for contact emails |
| `CONTACT_RECEIVER_EMAIL` | backend | Where contact emails land |
...
**users** — admin accounts only. No public registration.
...
| role | varchar(20) | defaults to `admin` |
| created_at | datetime | |

**blog_posts** — rich-text blog entries.

| Column | Type | Notes |
|---|---|---|
| id | integer | primary key |
| title | varchar(200) | |
| slug | varchar(200) | unique, auto-generated from title |
| excerpt | text | short preview |
| content | text | TipTap HTML or plain text |
| category | varchar(100) | comma-separated for multiple |
| content_type | varchar(20) | `rich`, `text`, or `html` |
| read_time | varchar(20) | auto-calculated on save |
| is_published | boolean | drafts hidden from public API |
| created_at | datetime | |
| updated_at | datetime | |

**case_studies** — project write-ups.
...
| category | varchar(50) | comma-separated |
...
```
GET  /portfolio/portfolio-data/data   All portfolio JSON (skills, experience, counters)
...
```
POST   /portfolio/auth/login          Get access + refresh tokens
...
GET    /portfolio/contact             All messages (paginated)
POST   /portfolio/upload              Upload image or video to Cloudinary
```

JWT access tokens expire after **15 minutes**. Refresh tokens last **7 days**. The frontend auto-refreshes on 401 responses — you shouldn't notice this in normal use.
...
Fetches all portfolio data from `/portfolio-data/data` in one request and passes it down. Sections: Hero with typewriter role animation, About with flip cards, Skills marquee, Counters, Experience timeline, and Featured Projects (separate fetch for `is_featured` case studies).

### Case Studies (`/case-studies`)

Lists all published case studies as project cards with thumbnail, title, description, and tech tags. Loads 6 at a time with a "Load more" button. No filtering — just chronological order.

### Case Study Detail (`/case-studies/:slug`)

Fetches a single case study by slug. Shows category badges, title, summary, tech stack tags, optional thumbnail image, and the full TipTap HTML content. Links to GitHub and live site if present. Uses React Query so navigating back from a detail page is instant.

### Blog (`/blog`)

Same pattern as case studies. Lists published blog posts with a search input that filters by title or category on the client side. 6 posts loaded initially, more on demand.

### Blog Detail (`/blog/:slug`)

Single blog post. Handles three content types: `rich` (dangerouslySetInnerHTML of TipTap HTML), `text` (plain paragraph), and `html` (raw HTML). Shows read time and publish date.

### Contact (`/contact`)

Two-column layout. Left column shows availability status, location/timezone, and social links. Right column has a contact form (name, email, message). Submissions hit the backend, which saves to the database and sends you an email via Resend.

### Admin (`/admin`) — protected

Requires a valid JWT. Three tabs: Blogs, Cases, and Messages.

- **Blogs/Cases**: Grid of content cards with edit and delete. "New" button opens a modal with the full form — TipTap rich editor for content, thumbnail upload for case studies, publish toggle.
- **Messages**: Read-only grid. Click any message card to open a detail modal with the full text, sender info, and a "Reply via Email" button that opens your mail client.

Pagination shows 9 items per tab. The modal uses `createPortal` to render into `document.body`, avoiding z-index issues with the main layout.

### Admin Login (`/admin/login`)

Email + password form. On success, stores `access_token` and `refresh_token` in localStorage. The axios interceptor handles token refresh automatically.

---

## Key frontend decisions

**React Query for data fetching.** Every public API call goes through a React Query hook in `useApiData.jsx`. Data stays cached for 5 minutes (list pages) or 10 minutes (detail pages). Navigating back to a page you've visited doesn't trigger a new request.

**Styled-components throughout.** No CSS files for component styles. Theming runs through a `ThemeProvider` with dark/light tokens. The active theme is stored in `localStorage` under `portfolio-theme`.

**Admin state is local.** The admin page manages blogs, cases, and messages in local `useState` — no React Query there, since you need to refetch immediately after mutations anyway.

**Cloudinary for uploads.** Images uploaded through the admin modal go directly to Cloudinary via the backend. The backend returns the `secure_url` which gets stored as a plain text URL in the database.
...
```
Set all environment variables in Render's dashboard. Use the Supabase pooled connection string.

**Frontend → Vercel**

Point Vercel at `frontend/my_portfolio`. Set `VITE_API_URL` to your Render backend URL. The `vercel.json` rewrite handles React Router, so page refreshes on any route work correctly.
...
You forgot to create the schema. Run `CREATE SCHEMA app_schema;` in your database before running `db.create_all()`.

**401 on every request after logging in**
Check that `VITE_API_URL` doesn't have a trailing slash and matches exactly what the backend expects. The axios base URL and the Flask route prefixes need to line up.
...
The upload route requires a valid JWT. If you're getting 401s on upload, your access token may have expired mid-session. The auto-refresh should catch this, but if it doesn't, log out and back in.

**`is_featured` not working for the home page**
Make sure you've toggled `is_featured` to true in the admin — it's a separate field from `is_published`. Both need to be true for a case study to appear in the featured section.
```
